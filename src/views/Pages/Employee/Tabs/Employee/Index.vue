<template>
  <div class="flex flex-col md:flex-row md:justify-between mb-4 mt-4">
    ...
    />
    <Add v-if="hasPermission('Add_Employee_Button')" @transaction_id="handleTransaction"/>
  </div>
  <div class="flex w-full overflow-auto">
    <table class="min-w-full divide-y divide-gray-200">
      ...
      <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm text-nowrap">
        <tr v-for="employee in data.employees" v-if="data.employees?.length > 0 && loading == false" :key="employee.id">
          ...
          <td class="px-6 py-4 whitespace-nowrap">
            <div class="flex justify-center self-start">
              ...
              <Edit v-if="hasPermission('Edit_Employee_Button')" :data="employee" @transaction_id="handleTransaction" />
              <Accountability v-if="hasPermission('Employee_Accountability_Button')" :data="employee" @transaction_id="handleTransaction" />
...
const listEmployee = async () => {
  try {
    ...
    const response = await axios.post(VUE_APP_API_URL+'employees/list', formData, BearToken(token));
