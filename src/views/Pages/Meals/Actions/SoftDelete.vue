<template>
  <button
    @click="deleteItem()"
    :class="[
      'bg-rose-600 text-white hover:opacity-70 focus:bg-blue-300 font-bold text-sm md:text-md py-2 px-4 float-end text-nowrap ',
      className,
    ]"
  >
...
const deleteItem = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to soft delete this Menu. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      const response = await axios.post(
        `${VUE_APP_API_URL}menus/soft-delete`,
        formData,
        BearToken(token)
      );
