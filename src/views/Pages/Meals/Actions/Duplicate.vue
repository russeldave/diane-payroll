<template>
  ...
  >
    <div class="grid grid-cols-1 gap-6 p-0 w-full">
      <form class="" @submit.prevent="duplicateMenu()" autocomplete="off">
        <div class="grid grid-cols-1 md:grid-cols-4">
          <div class="block bg-gray-600 p-4 md:min-h-screen">
          ...
          <div class="block md:col-span-3 p-4">
            ...
            >
              <div class="flex justify-end space-x-2">
                ...
                />
                <!-- <DeleteAllIngredient :key="menuForm.menu_id" :menu_id="menuForm.menu_id" @transaction_id="handleTransaction()" /> -->
              </div>
              <button
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="border p-2 text-center">
                      <!-- <button v-if="ingredient.exist" @click.prevent="deleteIngredient(ingredient.menu_ingredient_id)" class="bg-red-500 text-white p-1 rounded-full"><i class="fa fa-times"></i> Remove</button> -->
                      <button
                        @click.prevent="removeIngredient(index)"
                        class="bg-red-600 text-white p-1 rounded-full text-sm md:text-md"
                      >
...
const fillForm = () => {
  ...
  menuForm.value.name = data.title + " (Copy)";
...
const duplicateMenu = async () => {
  try {
    ...
    Swal.fire({
      title: "Processing...",
      text: "Please wait while we duplicate the menu.",
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}menus/duplicate`,
      formData,
      BearToken(token)
    );
