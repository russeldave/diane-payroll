...
const overrideRetailPrice = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to override menu retail price?. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.fire({
        title: "Processing...",
        text: "Please wait while we update the menu retail price.",
      ...
      const formData = FormDx({
        ...
        retail_price: props.data.price,
      });

      const response = await axios.post(
        `${VUE_APP_API_URL}override-food/menu-retail-price`,
        formData,
        BearToken(token)
      );
