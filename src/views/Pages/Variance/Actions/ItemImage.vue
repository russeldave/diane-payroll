<template>
  <button
    ...
    type="button"
    title="View Ingredient Images!"
    :class="[
      'bg-transparent text-white hover:opacity-70 hover:bg-blue-50 rounded font-bold text-sm md:text-md py-2 px-4 float-end text-nowrap ',
      className,
    ]"
  >
  ...
  >
    <div class="grid grid-cols-1 gap-6 p-6">
      ...
      <div v-else>
        <p class="text-center text-gray-500">No images available for this item.</p>
      </div>
    </div>
  </Modal>
</template>
...
const fetchImages = async () => {
  try {
    const response = await axios.post(
      `${VUE_APP_API_URL}items/get-images`,
      { warehouse_item_id: props.data.id },
      BearToken(token)
    );
