            ...
            >
              <td class="px-2 py-2 border text-center">
                <div class="flex flex-col">
                  ...
                  <div>
                    <span
                        class="rounded-lg px-2 py-1 text-md mt-1 font-bold"
              ...
              <td class="px-2 py-2 border">
                ...
                <button class="bg-red-600 p-2 rounded-lg text-white font-bold"
                  @click="rejectMrfTransafer()"  
                  >
                  Reject
                </button>
              </td>
...
const listMRFSlip = async () => {
  try {
    loading.value = true;
    const formData = FormDx(search.value);
    const response = await axios.post(
      `${VUE_APP_API_URL}mrfs/transfer-list`,
      formData,
      BearToken(token)
    );
...
const rejectMrfTransafer = () => {

}

// Call listMRFSlip when component is mounted
onMounted(() => {
  categoryDropdown();
  usersDropdown();
  listMRFSlip();
