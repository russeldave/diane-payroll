...
const deleteItem = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text:
        "You are about to delete this receiving transaction. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.fire({
        title: "Processing...",
        text: "Please wait while we delete the receiving transaction.",
      ...
      const formData = FormDx({ ingredient_receiving_id: props.data.id });
      const response = await axios.post(
        `${VUE_APP_API_URL}ingredient-receiving/delete`,
        formData,
        BearToken(token)
      );
