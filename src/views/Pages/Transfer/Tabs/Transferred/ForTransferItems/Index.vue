  ...
  <div v-if="hasPermission('Navigation_Transfer')">
    <div class="p-4 min-h-screen rounded-lg dark:border-gray-700">
      <div class="flex flex-col md:flex-row md:justify-between mb-4">
        ...
        >
          <AddIngredients @transaction_id="handleTransaction" />
        </div>
      </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm text-nowrap">
            ...
            >
              ...
              <td class="px-2 py-2 border text-wrap hover:bg-green-200">
                <ViewDetails
              ...
              >
                <u class="text-md md:text-lg"
                  >{{ mrf.quantity ?? 0 }} {{ mrf.unitAbbre }}</u
                >
              </td>
              <td
                class="px-2 py-3 border text-right hover:bg-green-200 border-white text-white bg-lime-500"
              >
                <!-- <small class="text-lg">Tancor 1</small> -->
                <p>{{ mrf.releasedQuantity ?? 0 }} {{ mrf.unitAbbre }}</p>
              </td>
              <!-- <td
              ...
              <td class="px-2 py-3 border">
                <div class="flex justify-center">
                  <ReleaseItems
                    v-if="hasPermission('Transfer_Items_Button')"
                    :isReleasing="true"
                    :key="mrf.id"
                    :warehouse_id="mrf.fromWarehouseId"
                    :mrf_item_id="mrf.id"
                    :data="mrf"
...
import ReleaseItems from "@/views/Pages/Transfer/Tabs/Transferred/ForTransferItems/Actions/ReleaseItems.vue";
...
const searchInput = useDebounce(async () => {
  ...
  await listForTransferItem();
  handlePagination(1);
}, 500);

const listForTransferItem = async () => {
  try {
    loading.value = true;
    const formData = FormDx(search.value);
    const response = await axios.post(
      `${VUE_APP_API_URL}mrfs/list-mrf-items-transfer-pending`,
      formData,
      BearToken(token)
    );
...
// Call listForTransferItem when component is mounted
onMounted(() => {
  categoryDropdown();
  usersDropdown();
  listForTransferItem();
