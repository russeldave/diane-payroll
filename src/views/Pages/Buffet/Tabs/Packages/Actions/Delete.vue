...
const deleteItem = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to delete this buffet. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.fire({
        title: "Processing...",
        text: "Please wait while we delete the buffet.",
      ...
      const response = await axios.post(
        `${VUE_APP_API_URL}buffets/delete-package`,
        formData,
        BearToken(token)
      );
