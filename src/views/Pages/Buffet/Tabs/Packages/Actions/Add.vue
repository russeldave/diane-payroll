<template>
  ...
  >
    <div class="grid grid-cols-1 gap-6 p-0">
      <form class="" @submit.prevent="addBuffetPackage()" autocomplete="off">
...
const defaultWarehouse = localStorage.getItem("warehouse_id");
...
const addBuffetPackage = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}buffets/add-package`,
      formData,
      BearToken(token)
    );
