<template>
  <button
    ...
    type="button"
    :class="[
      'bg-gray-500 text-white hover:opacity-70 focus:bg-blue-300 font-bold text-sm md:text-md py-2 px-4 float-end text-nowrap ',
      className,
    ]"
  >
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="right border p-2">
                      <div class="flex group">
                        <input
                          ...
                          class="bg-gray-200 border p-2 w-full text-left focus-visible:bg-yellow-100"
                        />
                        <!-- <button class="text-xs bg-red-500 p-2 text-white rounded-r" :title="menu.unit_name" v-if="ingredient.unit">{{ ingredient.unit }}</button> -->
                      </div>
                    </td>
                    <td class="right border p-2">
                      <div class="flex group">
                    ...
                    <td class="border p-2 text-center">
                      <!-- <button v-if="dish.exist" @click.prevent="deleteDish(dish.dishes_id)" class="bg-red-700 text-white p-1 rounded-full"><i class="fa fa-times"></i> Remove</button> -->
                      <button
...
const fillForm = () => {
  ...
  buffetForm.value.name = data.name + " (Copy)";
  buffetForm.value.description = data.description;
  buffetForm.value.date_from = "";
  buffetForm.value.date_to = "";
  buffetForm.value.buffet_retail_price = parseFloat(data.retailPrice);
...
const fillDishName = (item, index) => {
  ...
  if (buffetForm.value.dishes[index]) {
    ...
    buffetForm.value.dishes[index].dishes_id = item.dishesId ?? 0;
...
const getBuffetDishes = async () => {
  try {
    ...
    const response = await axios.post(
      VUE_APP_API_URL + "buffets/get-dishes",
      formData,
      BearToken(token)
    );
    ...
    buffetForm.value.dishes = response.data.menus.map((dish) => ({
      id: dish.id || 0, // Assuming 'id' exists in the response
      dishes_id: dish.dishesId || 0, // Assuming 'id' exists in the response
      buffet_id: dish.buffetId || 0, // Assuming 'buffet_id' exists in the response
      dishes_name: dish.dishesName || "", // Assuming 'name' exists in the response
      quantity: parseFloat(dish.dishesQuantity || 1), // Default to 1 if no quantity is provided
      cost_price: parseFloat(dish.dishesCostPrice || 1), // Default to 1 if no quantity is provided
      total: parseFloat(dish.totalCostPrice || 0), // Default to 0 if no quantity is provided
...
const editBuffet = async () => {
  try {
    ...
    Swal.fire({
      title: "Processing...",
      text: "Please wait while we duplicate the buffet.",
    ...
    buffetForm.value.cost_price = totalCost;
    const formData = FormDx(buffetForm.value);
    // Append image files
    imageFiles.value.forEach((file) => {
      formData.append("buffet_images[]", file); // Using array notation for multiple files
    });
...
const deleteDish = async (dish_id) => {
  try {
    ...
    if (confirmed.isConfirmed) {
      ...
      const formData = FormDx({ buffet_has_dish_id: dish_id });
      const response = await axios.post(
        `${VUE_APP_API_URL}buffets/delete-dish-in-buffet`,
        formData,
        BearToken(token)
      );
