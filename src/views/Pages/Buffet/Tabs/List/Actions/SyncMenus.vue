<template>
  ...
  >
    ...
    Sync
    <label for="" title="Ingredients Count" class="absolute -top-2 right-0 bg-red-600 text-nowrap uppercase text-xs rounded-full p-1 text-white" v-if="menuForm.total_ingredients > 0">{{ menuForm.total_ingredients }}</label>
  </button>
  <!-- Add Item Modal -->
  <Modal :show="isSyncModalOpen" :maxWidth="'4xl'" title="Sync Ingredients" @close="isSyncModalOpen = false">
    <div class="grid grid-cols-1 gap-6 p-6">
      <form autocomplete="off">
            <div class="mb-4">
              ...
              <small v-if="loading" class="text-red-500">Fetching Buffet...</small>
            </div>
            <div class="mb-4">
                <label
                    for="ProductName"
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full bg-white border border-gray-300">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr v-if="menuForm.ingredients?.length == 0 && !loading" class="hover:bg-gray-200">
                            ...
                            <tr v-for="ingredient in menuForm.ingredients" v-if="menuForm.ingredients?.length > 0 && !loading" :key="ingredient.id" class="hover:bg-gray-200">
                                <!-- <td class="px-6 py-4">
                                <div class="h-32 w-32">
                                ...
                                <td class="px-6 py-4 text-right border">{{ ingredient.cost_price ?? 0 }}</td>
                                <td class="px-6 py-4 text-left border border-r-0">{{ ingredient.quantity }} {{ ingredient.unit }}</td>
...
const syncIngredients = async () => {
  try {
    ...
    Swal.fire({
      title: 'Syncing Ingredients...',
      text: 'Please wait while the ingredients are being synced.',
      icon: 'info',
    ...
    // Simulate the syncing process (replace with your actual logic)
    emits('ingredients', menuForm.value.ingredients);
    ...
    Swal.fire({
      title: 'Success!',
      text: 'Ingredients synced successfully.',
      icon: 'success',
    ...
    Swal.fire({
      title: 'Error!',
      text: 'There was an issue syncing the ingredients.',
      icon: 'error',
      showConfirmButton: true
    });
  }
...
const getMealIngredients = async () => {
  try {
    ...
    const response = await axios.post(VUE_APP_API_URL + 'menus/get-ingredients', formData, BearToken(token));
