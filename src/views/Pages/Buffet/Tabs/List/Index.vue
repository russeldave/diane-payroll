      ...
      <div class="flex w-full overflow-auto">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <thead class="text-white shadow text-nowrap" :class="DEFAULT_BG">
            <tr class="border-b-2 border-solid border-yellow-500">
              ...
              <th
                scope="col"
                class="px-6 py-3 text-left text-xs md:text-sm font-medium uppercase border tracking-wider"
                @click="handleSort('dateFrom')"
              >
                From
                <i :class="getSortIcon('dateFrom')" class="ml-1"></i>
              </th>
              <th
                scope="col"
                class="px-6 py-3 text-left text-xs md:text-sm font-medium uppercase border tracking-wider"
                @click="handleSort('dateTo')"
              >
                To
                <i :class="getSortIcon('dateTo')" class="ml-1"></i>
              </th>
              <th
                scope="col"
          ...
          <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm">
            ...
            <tr
              v-for="buff in data.buffets"
              class="hover:bg-gray-200"
              v-if="data.buffets?.length > 0"
              :title="`Updated by: ${buff.userName ?? 'None'}`"
              :key="buff.id"
            >
              <td class="px-2 py-2 border text-left capitalize">{{ buff.name ?? 0 }}</td>
              <td class="px-2 py-2 border text-left uppercase">
                {{ buff.buffetCategoryName ?? "none" }}
              </td>
              <!-- <td class="px-2 py-2 border text-left uppercase">
                {{ buff.warehouseName ?? "none" }}
              </td> -->
              <td class="px-2 py-2 border text-left text-wrap">
                {{ buff.dateFrom ?? 0 }}
              </td>
              <td class="px-2 py-2 border text-left text-wrap">{{ buff.dateTo ?? 0 }}</td>
              <td class="px-2 py-2 border text-right">{{ buff.retailPrice }}</td>
              <td class="px-2 py-2 border">
                <div class="flex justify-center text-nowrap">
                  <!-- <View v-if="hasPermission('View_Buffet_Button')" :key="buff.id" :data="buff" @transaction_id="handleTransaction" /> -->
                  <Menu
                    v-if="hasPermission('Menu_Buffet_Button')"
                    :key="buff.id"
                    :data="buff"
                    @transaction_id="handleTransaction"
                  />
                  <Edit
                    v-if="hasPermission('Edit_Buffet_Button')"
                    :key="buff.id"
                    :data="buff"
                    @transaction_id="handleTransaction"
                  />
                  <Duplicate
                    v-if="hasPermission('Duplicate_Buffet_Button')"
                    :key="buff.id"
                    :data="buff"
                    @transaction_id="handleTransaction"
                  />
                  <Delete
                    v-if="hasPermission('Delete_Buffet_Button')"
                    :key="buff.id"
                    :data="buff"
                    @transaction_id="handleTransaction"
                  />
