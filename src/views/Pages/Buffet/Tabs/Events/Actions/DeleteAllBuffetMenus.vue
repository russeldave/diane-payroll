<template>
  <button
    @click.prevent="removeAllBuffetMenu()"
...
const removeAllBuffetMenu = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text:
        "You are about to remove all menu from this buffet. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.fire({
        title: "Processing...",
        text: "Please wait while we remove all menus.",
      ...
      const response = await axios.post(
        `${VUE_APP_API_URL}buffets/delete-all-dish-in-buffet`,
        formData,
        BearToken(token)
      );
