...
const overrideMenuName = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to override buffet name?. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.fire({
        title: "Processing...",
        text: "Please wait while we update the buffet name.",
      ...
      const formData = FormDx({
        buffet_id: props.data.buffet_id,
        buffet_name: props.data.name,
      });

      const response = await axios.post(
        `${VUE_APP_API_URL}override-food/buffet-name`,
        formData,
        BearToken(token)
      );
