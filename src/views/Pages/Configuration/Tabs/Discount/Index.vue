    ...
    <div class="flex w-full overflow-auto px-2">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm text-nowrap">
          ...
          <tr v-for="pm in data.discounts" v-if="data.discounts?.length > 0 && loading == false" :key="pm.id">
            <td class="px-2 py-2 border">{{ pm.name }} ({{ pm.abbre }})</td>
            <td class="px-2 py-2 border">{{ pm.remarks ?? '-' }}</td>
            <td class="px-2 py-2 border text-right">{{ pm.value ?? 0 }}</td>
            <td class="px-2 py-2 border">
              <div class="flex justify-center">
               <Delete @transaction_id="handleTransaction" :data="pm" />
              </div>
            </td>
          </tr>
...
const listDiscounts = async () => {
  try {
    ...
    const response = await axios.post(`${VUE_APP_API_URL}discounts/list`,formData,BearToken(token));
