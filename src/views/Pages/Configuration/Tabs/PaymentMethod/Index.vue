  ...
  >
    <div class="flex flex-col lg:flex-row lg:justify-between px-2">
      <input
        ...
        @input.prevent="listPaymentMethod()"
    ...
    <div class="flex w-full overflow-auto px-2">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm text-nowrap">
          ...
          <tr v-for="pm in data.paymentMethods" v-if="data.paymentMethods?.length > 0 && loading == false" :key="pm.id">
            <td class="px-2 py-2 border">{{ replaceUnderScore(pm.name) }}</td>
...
const listPaymentMethod = async () => {
  try {
    ...
    const response = await axios.post(`${VUE_APP_API_URL}payment-methods/list`,formData,BearToken(token));
