<template>
  <button
    @click.prevent="overrideWarehouseName()"
...
const overrideWarehouseName = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to override warehouse name?. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.fire({
        title: "Processing...",
        text: "Please wait while we update the warehouse name.",
      ...
      const formData = FormDx({
        ...
        warehouse_name: props.data.warehouse_name,
      });

      const response = await axios.post(
        `${VUE_APP_API_URL}overrides/warehouse-name`,
        formData,
        BearToken(token)
      );
