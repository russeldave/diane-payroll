<template>
  <div v-if="hasPermission('Navigation_Meals_Category')">
    <div class="px-4 min-h-screen rounded-lg dark:border-gray-700 bg-white">
      <!-- <BreadCrumbs :page="'Menu Category'" /> -->
      <h1 class="text-lg lg:text-2xl font-bold mb-4 mt-1">Menu Category Management</h1>
      ...
      <div class="flex w-full overflow-auto">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm">
            ...
            <tr
              v-for="ing in data.menuCategories"
              class="hover:bg-gray-200"
              v-if="data.menuCategories?.length > 0 && loading == false"
              :key="ing.id"
            >
...
const listCategories = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}menu-categories/list`,
      formData,
      BearToken(token)
    );
