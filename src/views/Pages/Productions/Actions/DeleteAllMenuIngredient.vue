...
const removeAllMenuIngredient = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text:
        "You are about to remove all ingredients from production. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      const formData = FormDx({ ingredient_id_marination: props.menu_id });
      const response = await axios.post(
        `${VUE_APP_API_URL}ingredients/delete-all-sub-ingredients-in-marination`,
        formData,
        BearToken(token)
      );
