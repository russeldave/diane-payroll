<template>
  <button
    @click.prevent="removeMenuIngredient()"
...
const removeMenuIngredient = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text:
        "You are about to remove this ingredient from production. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.fire({
        title: "Processing...",
        text: "Please wait while we remove the ingredient.",
      ...
      const formData = FormDx({ menu_ingredient_id: props.data.id });
