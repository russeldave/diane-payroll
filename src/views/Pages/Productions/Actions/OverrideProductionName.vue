<template>
  <button
    ...
    title="Override Production Name"
...
const overrideMenuName = async () => {
  try {
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.fire({
        title: "Processing...",
        text: "Please wait while we update the production name.",
      ...
      const formData = FormDx({
        ingredient_id_marination: props.data.ingredient_id_number,
        ingredient_name: props.data.name,
      });

      const response = await axios.post(
        `${VUE_APP_API_URL}override-food/marination-name`,
        formData,
        BearToken(token)
      );
