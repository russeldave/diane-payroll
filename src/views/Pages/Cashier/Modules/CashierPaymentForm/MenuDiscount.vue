    ...
    <div class="flex justify-center space-x-2 mt-2">
      ...
      <div class="block">
        ...
        <div class="flex space-x-3 flex-wrap">
          ...
          <button
            @click.prevent="computeDiscount(0)"
      ...
      <div class="mt-4 flex justify-center space-x-4">
        <div class="block">
          ...
          <small
            ...
            ><i class="fa fa-spinner animate-spin"></i>&nbsp;Calculating
            Discount...</small
          >
        </div>
      </div>
...
grandTotal.value = props.data?.grand_total;
watch(
  [() => props.data?.grand_total, () => props.data?.discount_value],
  ([newGrandTotal, newDiscountValue], [oldGrandTotal, oldDiscountValue]) => {
    grandTotal.value = newGrandTotal;
    menuDiscount.value.final_price = grandTotal.value;
    menuDiscount.value.discount_value = newDiscountValue;

    console.log(
      `GrandTotal changed: ${oldGrandTotal} → ${newGrandTotal}, Discount: ${oldDiscountValue} → ${newDiscountValue}`
    );
  }
);
...
const menuDiscount = ref({
  ...
  quantity: 0,
  price: 0,
  discount_id: props.data?.discount_id ?? 0,
  discount_type: "percent", // either 'percent' or 'amount'
  value: props.data?.discount_value ?? 0,
  discounted_price: 0,
  discount_abbre: props.data?.discount_abbre ?? 0,
  final_total: grandTotal.value,
  less: props.data?.less ?? 0,
  service_fee: props.data?.service_fee ?? 0, // optional chaining and default
  service_fee_enabled: props.data?.service_fee ?? 0,
});
...
const showMenuDiscount = () => {
  const item = props.data; //item.service_fee;
  ...
  menuDiscount.value.final_price = grandTotal.value;
  menuDiscount.value.service_fee = item.service_fee;
  menuDiscount.value.service_fee_enabled = item.service_fee_enabled;
...
const undoDiscountToMenu = () => {
  ...
  // console.log("Test for discount remove");
  // console.log(menuDiscount);
  emits("discountData", menuDiscount.value);
};
...
const applyDiscountValue = async () => {
  try {
    ...
    if (response) {
      ...
      menuDiscount.value.service_fee = parseFloat(discountedData.serviceFee ?? 0);

      // console.log("TEst for transaction discount computation");
      // console.log(menuDiscount);
      emits("discountData", menuDiscount.value);
      // Alert("success", "Success!", "Order Discount Applied");
    }
