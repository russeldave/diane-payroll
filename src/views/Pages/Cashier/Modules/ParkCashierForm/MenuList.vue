<template>
  ...
  <div class="w-full mb-4 p-0 relative h-full flex flex-col">
    ...
    <!-- Scroll Indicators -->
    <div v-if="isScrollable && !isScrolledToBottom" class="absolute bottom-0 left-1/2 transform -translate-x-1/2 animate-bounce">
      <i class="fa fa-chevron-down text-gray-400"></i>
    </div>
    <div v-if="isScrollable && isScrolledToBottom" class="absolute top-0 left-1/2 transform -translate-x-1/2 animate-bounce">
...
const applyDiscount = async () => {
  try {
    ...
    if (response) {
      ...
      const activeIndex = menuDiscount.value.index;
      items.value[activeIndex].discount_value = discountedData.less ?? 0;
