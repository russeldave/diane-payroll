<template>
  <button
    ...
    type="button"
    title="Upload Utak Excel File"
    :class="[
      'bg-green-500 text-white px-4 py-2 rounded-r ml-1 hover:opacity-70  focus:bg-blue-300 font-bold text-sm md:text-md rounded ',
      className,
    ]"
  >
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm">
              ...
              <tr
                ...
                :key="ing.id"
              >
                <td class="px-2 py-2 border">{{ ing.item }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const uploadUtakInventory = async () => {
  ...
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}menus-excel/upload-menus`,
      formData,
      BearToken(token),
      {
    ...
    if(error.response.data.message == "NotFound" || error.response.data.message == "Duplicates"){
      isUploadError.value = true;
      console.log("isUploadError is true");
      ErrorMessage.value = error.response.data.errorMessage
      incompleteIngredients.value = error.response.data.lists
    }else{
