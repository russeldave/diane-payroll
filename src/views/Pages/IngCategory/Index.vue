<template>
  <div v-if="hasPermission('Navigation_Ingredient_Category')">
    <div class="px-4 min-h-screen rounded-lg dark:border-gray-700 bg-white">
      <!-- <BreadCrumbs :page="'Ingredient Category'" /> -->
      <h1 class="text-lg lg:text-2xl font-bold mb-4 mt-1">
        Ingredient Category Management
      </h1>
      <div class="flex flex-col lg:flex-row md:justify-between mb-4">
        <input
          type="search"
        ...
        />
        <div class="flex flex-row justify-start md:justify-end items-center space-x-2">
          <!-- <SubCategory @transaction_id="handleTransaction" /> -->
          <!-- <AddSub
            v-if="hasPermission('Add_Ingredient_Sub_Category_Button')"
            @transaction_id="handleTransaction"
          /> -->
          <Add
            v-if="hasPermission('Add_Ingredient_Category_Button')"
            @transaction_id="handleTransaction"
          />
        </div>
      </div>
      <div class="flex w-full overflow-auto">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm">
            ...
            <tr
              v-for="ing in data.ingredientCategories"
              class="hover:bg-gray-200"
              v-if="data.ingredientCategories?.length > 0 && loading == false"
              :key="ing.id"
            >
              <td class="px-2 py-2 border">{{ ing.name }} ({{ ing.abbre }})</td>
              <!-- <td class="px-2 py-2 border">{{ ing.subCategoryName ?? "-" }}</td> -->
              <td class="px-2 py-2 border">
                <div class="flex justify-center flex-nowrap">
                  <Edit
                    v-if="hasPermission('Edit_Category_Button')"
                    :data="ing"
                    @transaction_id="handleTransaction"
                  />
                  <Delete
                    v-if="hasPermission('Delete_Category_Button')"
                    :data="ing"
                    @transaction_id="handleTransaction"
                  />
                </div>
...
import AddSub from "./Actions/AddSub.vue";
import Edit from "./Actions/Edit.vue";
import Delete from "./Actions/Delete.vue";
import SubCategory from "./Actions/SubCategory.vue";
...
const listCategories = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}ingredient-categories/list`,
      formData,
      BearToken(token)
    );
