<template>
  <button
    ...
    class="bg-blue-400 text-white hover:opacity-70 focus:bg-blue-300 font-bold text-sm md:text-md py-2 rounded px-4 mb-4 float-end text-nowrap"
  >
    <i class="fa fa-history"></i> Production
  </button>
  <Modal
    :show="isViewModalOpen"
    :maxWidth="'4xl'"
    title="Search Ingredient Production"
    @close="closeModal()"
  >
      ...
      >
        <div class="flex w-full justify-between">
          <h1 class="text-lg font-bold text-gray-700">
            Production with this ingredients used.
          </h1>
        </div>
      </div>
      <div
      ...
      >
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr
              v-for="order in data.productions"
              class="hover:bg-gray-200"
              v-if="data.productions?.length > 0 && !loading"
              :key="order.id"
            >
              <td
                class="px-6 py-4 whitespace-nowrap text-left text-sm text-gray-900 border"
              >
                {{ order.ingredientIdNumber ?? "-" }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 border">
                {{ order.ingredientFullName }}
              </td>
            </tr>
...
const getProductionHistory = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}ingredients/search-in-productions`,
      formData,
      BearToken(token)
    );
    if (response.data) {
