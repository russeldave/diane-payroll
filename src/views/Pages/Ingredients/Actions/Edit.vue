<template>
  <button
    ...
    type="button"
    :class="[
      'bg-yellow-500 text-white hover:opacity-70 focus:bg-blue-300 font-bold text-sm md:text-md py-2 px-4 float-end text-nowrap hover:scale-110',
      className,
    ]"
  >
      ...
      <form class="mt-4" @submit.prevent="editIngredient()" autocomplete="off">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <!-- <div class="mb-1 md:col-span-2">
                <div class="h-32 w-32">
                  <IngImageViewer v-if="ingredientForm.ingredient_id" :key="ingredientForm.ingredient_id" :ingredient_id="ingredientForm.ingredient_id" :thumbnail="ingredientForm.thumb_nail" :canDeleteImage="true"/>
                </div>
          </div> -->
          ...
          <div>
            <input
              type="checkbox"
                v-model="ingredientForm.restore_item"
          ...
          <!-- @click.prevent -->
          <AddToZohoBooks 
                :key="ingredientForm.id"
                :ingredient_id_number="ingredientForm.ingredient_id_number"
          />
          ...
          <div class="mb-1 md:col-span-2 border border-2 border-orange-200 rounded-lg border-b-4">
            <label for="ProductName" class="block text-md font-bold px-2"
            ...
            <div class="flex mt-1">
              ...
              />
              <OverrideIngredientName
                v-if="hasPermission('Override_Ingredient_Name')"
                :key="ingredientForm.ingredient_name"
                :data="ingredientForm"
              />
            </div>
          </div>
          <div class="mb-1">
            <label for="ProductName" class="block text-md font-bold"
              >Category</label
            >
            <select
              class="mt-1 p-2 border rounded-md w-full"
          ...
          <div class="mb-1 md:col-span-2 border border-2 border-orange-200 border-b-4 rounded-lg">
            <label for="ProductName" class="block text-md font-bold px-2"
              >Cost Price</label
            >
            <div class="flex mt-1">
              ...
              <OverrideCostPrice
                ...
                :key="ingredientForm.cost_price"
                :ingredient_id="ingredientForm.ingredient_id_number"
                :cost_price="ingredientForm.cost_price"
              />
            </div>
          </div>
          <div class="mb-1 md:col-span-2 border border-2 border-orange-200 border-b-4 rounded-lg">
            ...
            <div class="flex mt-1">
              <input
                type="text"
                id="ProductName"
                v-model="ingredientForm.sku"
                :disabled="!hasPermission('Override_Sku')"
                placeholder="Input Ingredient Name"
                :class="
                  hasPermission('Override_Sku')
                    ? 'bg-white rounded-l'
                    : ' bg-gray-200 rounded'
                "
                class="p-2 border rounded-l w-full bg-gray-200"
          ...
          <div class="mb-1 md:col-span-2 border border-2 border-orange-200 border-b-4 rounded-lg">
            ...
            <div class="flex mt-1">
              <input
                type="text"
                id="ProductName"
                v-model="ingredientForm.zoho_item_id"
                placeholder="Input Ingredient Name"
              ...
              <OverrideZohoItemId
                ...
                :key="ingredientForm.id"
                :ingredient_id="ingredientForm.ingredient_id_number"
                :zoho_item_id="ingredientForm.zoho_item_id"
              />
            </div>
          </div>
          <div class="mb-1 md:col-span-2 border border-2 border-orange-200 border-b-4 rounded-lg">
            <label for="ProductName" class="block text-md font-bold px-2"
              >From Recieiving Auto stocking in Warehouse: </label
            >
            <div class="flex mt-1">
              <select
                id="warehouseName"
                v-model="ingredientForm.auto_stocking_warehouse_id"
                class="mt-1 p-2 border rounded-md w-full"
                >
              ...
              <!-- v-if="hasPermission('Override_Cost_Price')" -->
              <OverrideAutoAssignWarehouse
              
                @transaction_id="handleTransaction"
                :key="ingredientForm.id"
                :ingredient_id="ingredientForm.ingredient_id_number"
                :auto_assign_warehouse_id="ingredientForm.auto_stocking_warehouse_id"
                :data="ingredientForm"
              />
            </div>
          </div>
          ...
          <div class="mb-4 md:col-span-2">
            ...
            <select
              id="warehouseName"
              v-model="ingredientForm.warehouse_id"
              class="mt-1 p-2 border rounded-md w-full"
            >
              <option value="0">--Select Warehouse--</option>
              <option
        ...
        <div class="flex items-center justify-center my-6 mt-0" v-if="props.unpack">
          <div class="flex-grow border-t border-gray-400"></div>
          <span class="mx-4 text-gray-400 text-sm font-semibold">Unpack Details</span>
          <div class="flex-grow border-t border-gray-400"></div>
        </div>
        <div class="grid grid-cols-1 gap-6" v-if="props.unpack">
          <div class="mb-1">
            <label for="ProductName" class="block text-sm font-medium text-gray-700"
            ...
            <div class="flex items-center mb-2">
              <SearchDropdown
                ...
                className="focus-visible:bg-yellow-100 p-2 w-full mt-1 border"
              ...
              <button
                type="button"
                @click.prevent="removeUnpackIngredient()"
                class="bg-red-500 p-2 mt-1 text-white rounded-r"
              >
...
import OverrideZohoItemId from "../Actions/OverrideZohoItemId.vue";
import AddToZohoBooks from "../Actions/AddToZoho.vue";
import IngImageViewer from "../Actions/IngImageViewer.vue";
import OverrideAutoAssignWarehouse from "../Actions/OverrideAutoAssignWarehouse.vue";
...
const fillForm = () => {
  ...
  ingredientForm.value.warehouse_id = data.warehouseId ?? defaultWarehouse.value;
  ingredientForm.value.restore_item = data.status === 7 ? false : true;
  ingredientForm.value.zoho_item_id = data.zohoItemId;
  ingredientForm.value.sku = data.sku;
  ingredientForm.value.auto_stocking_warehouse_id = data.autoAssignWarehouseId ?? 0;
};
...
const resetForm = () => {
  ...
  // console.log(ingredientForm.value.restore_item)
};
...
const removeUnpackIngredient = () => {
  ingredientForm.value.unpack_ingredient_id = 0;
  ingredientForm.value.unpack_ingredient_name = "";
  ingredientForm.value.unpack_unit_abbre = ""; // New field for unit abbreviation
  ingredientForm.value.unpack_quantity = 0; // New field for unpack quantity

  Alert("success", "Success", "Unpack Ingredient Removed Successfully!");
};
//fillIngredientForm
...
const editIngredient = async () => {
  try {
    ...
    Swal.fire({
      title: "Processing...",
      text: "Please wait while we update the ingredient.",
...
const restoreItem = async () => {
  ...
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}ingredients/restore`,
      formData,
      BearToken(token)
    );
