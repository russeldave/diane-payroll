...
const search = ref({
  ...
  ingredient_id_number: props.ingredientIdNumber, // Default to current user
});
...
const getAllHistory = async () => {
  // console.log('GetAllHistory in line 225');
  // console.log(props.ingredientIdNumber)
  try {
    loading.value = true;
    const formData = FormDx(search.value);
    const response = await axios.post(
      `${VUE_APP_API_URL}ingredients/all-history`,
      formData,
      BearToken(token)
    );
     if (response.data) {
...
watch(
  () => props.activeTTab,
  (newVal) => {
    if (newVal === "allHistory") {
      getAllHistory();
    }
    console.log("Switches to all history tab")
  }
);

onMounted(() => {
  if (props.show) getAllHistory();
});
</script>
