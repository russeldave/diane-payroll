<template>
  <button
    @click.prevent="menusBehavior()"
    type="button"
    :class="[
      'bg-[#540863] text-white hover:opacity-70 focus:bg-blue-300 font-bold text-sm md:text-md py-2 px-4 float-end text-nowrap hover:scale-110',
      className,
    ]"
  >
      ...
      <div class="block mt-4 border-t-2 border-dashed">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr
              v-for="(i, ii) in menus?.menus"
              v-if="menus?.menus?.length > 0"
              :key="ii"
              :class="i.menuStatus === 7 ? 'bg-slate-400' : ''"
            >
              <td class="px-2 py-2 border">
                {{ i.warehouseName }}
              </td>
              <td class="px-2 py-2 border text-right font-bold">
                {{ i.menu }} - &nbsp;
              ...
              <td class="px-2 py-2 border text-right italic">
                ₱{{ i.menuRetail }}
              </td>
            </tr>
            <tr v-else>
...
const listMenus = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}ingredients/get-menus`,
      formData,
      BearToken(token)
    );
