<template>
  <button
    @click="deleteItem()"
    :class="[
      'bg-green-600 text-white hover:opacity-70 focus:bg-blue-300 font-bold text-sm md:text-md py-2 px-4 float-end text-nowrap hover:scale-110',
      className,
    ]"
  >
...
const deleteItem = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to Restore this item. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.close();
      Alert("warning", "Restored!", response.data.message);
