<template>
  <button
    @click.prevent="overrideZohoItemId()"
...
const overrideZohoItemId = async () => {
  try {
    ...
    if (confirmed.isConfirmed) {
      ...
      const formData = FormDx({
        ingredient_id_number: props.ingredient_id,
        zoho_item_id: props.zoho_item_id,
      });

      const response = await axios.post(
        `${VUE_APP_API_URL}override-food/edit-zoho-item-id`,
        formData,
        BearToken(token)
      );
