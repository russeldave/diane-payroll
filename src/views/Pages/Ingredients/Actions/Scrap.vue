<template>
  <button
    ...
    class="bg-orange-500 text-white hover:opacity-70 focus:bg-orange-300 font-bold text-sm md:text-md py-2 px-4 float-end text-nowrap hover:scale-110"
  >
  ...
  >
    ...
    <div class="p-2">
      ...
      <!-- Scrap List Tab -->
      <div v-else-if="activeTab === 'list'" class="px-3">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold text-gray-700">Scrap History</h3>
          <input
            type="text"
...
const getScrapList = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}scrap-ingredients/list`,
      formData,
      BearToken(token)
    );
...
const fillForm = () => {
  ...
  ingredientForm.value = {
    ...
    ingredient_id_number: data.ingredientIdNumber ?? "",
