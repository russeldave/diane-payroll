<template>
  <button
    @click.prevent="overrideIngredientName()"
...
const overrideIngredientName = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to override ingredient name?. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.fire({
        title: "Processing...",
        text: "Please wait while we update the ingredient name.",
      ...
      const formData = FormDx({
        ingredient_id: props.data.ingredient_id_number,
        ingredient_name: props.data.ingredient_name,
        ingredient_id_number: props.data.ingredient_id_number,
      });

      const response = await axios.post(
        `${VUE_APP_API_URL}override-food/ingredient-name`,
        formData,
        BearToken(token)
      );
