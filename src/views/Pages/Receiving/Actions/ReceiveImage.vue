<template>
  <button
    ...
    type="button"
    :class="[
      'bg-blue-500 text-white hover:opacity-70 rounded-l focus:bg-blue-300 font-bold text-sm md:text-md py-2 px-4 float-end text-nowrap ',
      className,
    ]"
  >
  ...
  <Modal
    ...
    :maxWidth="'xl'"
    title="View Received Ingredient Images"
...
const fetchImages = async () => {
  try {
    const response = await axios.post(
      `${VUE_APP_API_URL}ingredient-receiving/get-images`,
      { receive_item_id: props.data.id },
      BearToken(token)
    );
