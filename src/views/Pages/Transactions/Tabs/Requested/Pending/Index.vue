      ...
      <div class="flex w-full overflow-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm">
            ...
            >
            <td class="px-6 py-4 border text-center align-middle">
            <div class="flex flex-col items-center space-y-2">
              <!-- Thumbnail -->
              <div class="w-20 h-20 overflow-hidden rounded-lg shadow-md bg-gray-100">
                <ItemImageViewer
                  v-if="item.id"
                  :key="item.id"
            ...
            <td class="px-2 py-2 border text-center">
              <div class="flex justify-center">
                <ApproveFill
                  v-if="
                    hasPermission('Requested_Items_Approve_Button') && item.status == 1
                  "
                  :data="item"
                ...
                <Reject
                  v-if="
                    hasPermission('Requested_Items_Reject_Button') && item.status == 1
                  "
                  :data="item"
                  @transaction_id="handleTransaction"
                />
                <Undo
                  v-if="
                    hasPermission('Requested_Items_Undo_Button') &&
...
const listRequestedItems = async () => {
  try {
    ...
    const response = await axios.post(
      VUE_APP_API_URL + "user-requests/list-pending",
      formData,
      BearToken(token)
    );
