<template>
  <button
    ...
    type="button"
    class="bg-gray-500 hover:opacity-70 focus:bg-gray-300 text-white font-bold rounded text-sm md:text-md py-2 px-4 mb-4 float-end text-nowrap"
  >
...
const cancelItem = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: "Do you want to cancel this pr item?. This action cannot be undone.",
      icon: "question",
    ...
    if (confirmed.isConfirmed) {
      ...
      const response = await axios.post(
        `${VUE_APP_API_URL}prs/cancel-pr-item`,
        formData,
        BearToken(token)
      );
