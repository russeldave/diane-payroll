<template>
  <button
    ...
    type="button"
    class="bg-orange-500 hover:opacity-70 focus:bg-orange-600 text-white font-bold rounded-l text-sm md:text-md py-2 px-4 mb-4 float-end text-nowrap"
  >
  ...
  try {
    ...
    // if (confirmed.isConfirmed) {
      ...
      const response = await axios.post(
        `${VUE_APP_API_URL}prs/pr-item-change-to-purchase`,
        formData,
        BearToken(token)
      );
