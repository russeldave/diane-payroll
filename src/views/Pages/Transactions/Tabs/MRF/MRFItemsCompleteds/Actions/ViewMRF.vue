<template>
  ...
  <!-- Add Item Modal -->
  <Modal :show="isEditModalOpen" :maxWidth="'6xl'" title="Item Releasing" @close="isEditModalOpen = false">
      ...
      <form class="mt-4">
        ...
        <div class="flex w-full overflow-auto">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <!-- Table headers -->
            <thead class=" shadow text-nowrap" :class="DEFAULT_BG">
              <tr
                class="border-b-2 border-solid border-yellow-500 text-white"
              >
                <th
                  scope="col"
                  class="px-6 py-3 text-left text-md font-medium uppercase border tracking-wider"
                >
                  Item SKU
                </th>
                <th
                ...
                <th
                  scope="col"
                  class="px-6 py-3 text-right text-md font-medium uppercase border tracking-wider"
                >
                  Stocks
                </th>
                <th
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              >
                ...
                <td class="px-2 py-2 border text-right">
                  {{ item.quantityPerUnit }}/{{ item.unitAbbre }}
                </td>
                <td class="px-2 py-2 border text-right">
                  {{
        ...
        <div v-if="proof && proof.length" class="mt-4">
          <label
              for="LeagueName"
              class="block text-sm font-medium text-gray-700"
              >Proof Images</label
            ...
            <div class="grid grid-cols-5 gap-1">
                  <div v-for="(i, index) in proof" :key="index">
...
const getProof = async () => {
  try {
    proof.value = [];
    const formData = FormDx({mrf_item_id: itemsForm.value.id});
    const response = await axios.post(
      `${VUE_APP_API_URL}mrfs/get-mrf-item-proof-images`,
      formData,
      BearToken(token)
    );
    proof.value = response.data.images;
  } catch (error) {
    handleApiError(error);
  }
};
