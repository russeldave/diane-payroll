        ...
        <thead class="text-white shadow text-nowrap" :class="DEFAULT_BG">
          <tr class="border-b-2 border-solid border-yellow-500">
            ...
            <th
              scope="col"
              width="15%"
              class="px-2 py-3 border text-cneter text-xs md:text-sm font-medium uppercase tracking-wider"
            >
              MRF'ed By
            </th>
            <th
...
import View from "../MRFSlipForReleasing/Actions/View.vue";
import Return from "../MRFSlipForReleasing/Actions/Return.vue";
...
const listMRFs = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}mrfs/list-completeds`,
      formData,
      BearToken(token)
    );
