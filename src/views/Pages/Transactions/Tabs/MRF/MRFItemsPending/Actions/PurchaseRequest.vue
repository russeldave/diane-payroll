...
const prForm = ref({
  ...
  estimated_arrival: "", // Added estimated_arrival field
  remarks: "",
  pr_manual_number: "",
  pr_price: 0,
  items: [],
...
const addPRBehavior = () => {
  ...
  warehouseDropdown();
  // dataa =  props.data;
  prForm.value.items = Array.isArray(props.data)
    ? props.data.map(item => ({
        ...item, // Deep copy for reactivity
        quantity: Number(item.requested_quantity) || 0, // Ensure numeric, fallback to 0
      }))
    : [];

  // prForm.value.items = dataa;
...
const resetForm = () => {
  ...
  prForm.value.requested_to_user_id = 0;
  prForm.value.estimated_arrival = ""; // Reset estimated_arrival field
  prForm.value.items = [];
};
const addToPR = async () => {
  console.log("addToPR");
