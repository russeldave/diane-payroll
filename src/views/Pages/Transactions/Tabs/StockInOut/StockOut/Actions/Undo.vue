<template>
    <button
      ...
      class="bg-gray-500 hover:opacity-70 focus:bg-gray-300 text-white rounde-l font-bold rounded text-sm md:text-md py-2 px-4 mb-4 float-end text-nowrap"
    >
  ...
  const undoRejectItem = async () => {
    try {
      ...
      if (confirmed.isConfirmed) {
        ...
        const formData = FormDx({ user_request_id: props.data.id, status_id: 1 });
        const response = await axios.post(
          `${VUE_APP_API_URL}user-requests/change-status`,
          formData,
          BearToken(token)
        );
