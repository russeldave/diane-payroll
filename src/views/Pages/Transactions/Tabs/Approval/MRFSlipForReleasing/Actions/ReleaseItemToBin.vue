<template>
  <button
    ...
    class="bg-red-500 hover:opacity-70 focus:bg-red-300 text-white font-bold text-sm md:text-md py-2 px-4 mb-4 float-end text-nowrap"
  >
    <i class="fas fa-trash"></i>
    Bin
  </button>
  <!-- Add Modal -->
  <Modal :show="assignBinModalOpen" :maxWidth="'6xl'" title="Release From Bin" @close="assignBinModalOpen = false">
    <div class="grid grid-cols-1 gap-6 p-6">
      <form class="mt-4" @submit.prevent="releaseItemFromBin()">
        <div
          v-if="assignForm.all_stocks == 0"
          class="mb-2"
        >
          <div
        ...
        >
          ...
          <div class="mb-4">
            ...
            >
            <!-- building_id,contact_info,address,geo_map,remarks. -->
            <p v-if="assignForm.mrf_type == 1" class="text-teal-500 font-bold mt-1 p-2 border bg-gray-200 rounded-md w-full">Request Item</p>
            <p v-else class="text-blue-500 font-bold mt-1 p-2 border bg-gray-200  rounded-md w-full">Transfer Item</p>
          </div>
          <div class="mb-4">
            <label
              for="LeagueName"
...
const binBehavior = () => {
  ...
  assignForm.value.current_bin_id = 0;
  assignForm.value.current_bin_name = data.binSku;
  assignForm.value.bin_id = 0;
  assignForm.value.bin_name = "";
  assignForm.value.rack_id = 0;
  assignForm.value.rack_name = data.rackName;
  ...
  try {
    ...
    const link_url =
      assignForm.value.is_fixed == 1
        ? "out-item-in-bin-fixed-asset"
        : "out-item-in-bin-fast-moving";
    ...
    if (result.isConfirmed) {
      ...
      const response = await axios.post(
        `${VUE_APP_API_URL}bin-in/${link_url}`,
        formData,
        BearToken(token)
      );
...
const listBins = async () => {
  try {
    ...
    if (!response.data.error) {
      ...
      assignForm.value.current_sub_quantity = response.data.bin.subStocks ?? 0;
...
//location dropdown
const fillFixedAssetLocationForm = (item) => {
  const buildingAbbre = item.buildingAbbre ? `(${item.buildingAbbre})` : '';;
  assignForm.value.location_sku = item.sku;
  assignForm.value.location_name =  item.sku + "-" + item.name+' '+buildingAbbre;
