...
const deleteItem = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: "Do you want to delete this mrf item?. This action cannot be undone.",
      icon: "question",
    ...
    if (confirmed.isConfirmed) {
      ...
      const formData = FormDx({ mrf_item_id: props.data.mrf_item_id });
      const response = await axios.post(
        `${VUE_APP_API_URL}mrfs/delete-mrf-item`,
        formData,
        BearToken(token)
      );
      Swal.close();
      Alert("success", "MRF Items Deleted!", response.data.message);
