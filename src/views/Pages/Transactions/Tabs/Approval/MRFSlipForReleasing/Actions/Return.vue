<template>
  <button
    @click.prevent="(isViewModalOpen = true), editMRFBehavior()"
    type="button"
    class="bg-gray-500 hover:opacity-70 focus:bg-gray-300 text-white font-bold text-sm md:text-md py-2 px-4 mb-4 float-end text-nowrap"
  >
        ...
        <div class="flex w-full overflow-auto p-2" v-if="isMobile">
          <div v-if="isMobile" class="block lg:hidden">
            <div
              ...
              class="relative mb-4 p-4 border rounded-lg shadow"
            >
              <!-- Item Dropdown -->
              <div class="absolute top-2 right-3">
                <div class="flex w-full">
                  <small class="text-red-500 font-bold mr-2">Return Item</small>
                  <input
                    type="checkbox"
              ...
              <!-- Remove Item Button -->
              <div class="flex justify-center border-t border-dashed pt-2">
                <DirectRelease
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200 text-sm">
              ...
              >
                ...
                <td
                  class="px-2 py-3 border text-right hover:bg-green-200 bg-red-500 font-bold"
                >
                  {{ mrf.quantity ?? 0 }}
                  {{ mrf.subUnitId }}
                </td>
                <td
                  class="px-2 py-3 border text-right hover:bg-green-200 border-white bg-lime-500 font-bold"
                >
                  {{ mrf.releasedQuantity ?? 0 }}
                  {{ mrf.unitAbbre }}
                </td>
                <td
                  class="px-2 py-3 border text-right hover:bg-green-200 border-white bg-green-500 font-bold"
                >
                  {{ mrf.unpaidQuantity ?? 0 }}
                  {{ mrf.unitAbbre }}
                </td>
                <td
                  class="px-2 py-3 border text-right hover:bg-green-200 bg-blue-500 font-bold"
                >
                  {{ mrf.allStocks }}
                  {{ mrf.unitAbbre }}
                </td>
...
import ReturnItem from "../../MRFItemsForReleasing/Actions/ReturnItem.vue";
...
const toggleDetails = (index) => {
  ...
  if (rowIndex > -1) {
    // Remove the index if it is already expanded
    expandedRows.value.splice(rowIndex, 1);
  } else {
    // Add the index if it is not expanded
