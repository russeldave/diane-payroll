      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <thead class="text-white shadow text-nowrap" :class="DEFAULT_BG">
          <tr class="border-b-2 border-solid">
            ...
            <th
              scope="col"
              class="w-50 px-2 py-3 border text-center text-xs md:text-sm font-medium uppercase"
            >
              Ingredients Name
            </th>
            <th
        ...
        <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm text-nowrap">
          ...
          >
            ...
            <td class="px-2 py-3 border text-left hover:bg-green-200 ">
              ...
              <p v-if="mrf.toEmployeeName" class="uppercase font-bold">To: {{ mrf.toEmployeeName }}</p>
            ...
            >
              <p>{{ Number(mrf.quantity).toLocaleString() }} {{ mrf.unitAbbre }}</p>
            </td>
            <td
              v-if="hasPermission('MRF_Available_Column')"
...
// controls
import PurchaseRequest from "../../MRF/MRFItemsPending/Actions/PurchaseRequest.vue";
import Completed from "../../MRF/MRFSlipForReleasing/Actions/Completed.vue";
import ReleaseItemToBin from "../../MRF/MRFSlipForReleasing/Actions/ReleaseItemToBin.vue";
import DirectRelease from "../../MRF/MRFSlipForReleasing/Actions/DirectRelease.vue";
...
const listMRFs = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}mrfs/list-mrf-items-for-approval`,
      formData,
      BearToken(token)
    );
