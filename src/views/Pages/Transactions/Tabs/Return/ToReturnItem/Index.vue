        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm text-nowrap">
            ...
            >
              ...
              >
                <ItemTitle :item_id="item.id" :item_name="item.name" />
              </td>
              <td
                class="px-2 py-2 border text-left font-bold bg-yellow-50 first-letter:capitalize text-wrap"
              >
                <ItemTitle :item_id="item.subItemId" :item_name="item.subItemName" />
              </td>
              <td class="px-2 py-2 border text-left">
                {{ typeFormatter(item.isFixed) }}
              </td>
              <td class="px-2 py-2 border text-left">{{ item.categoryName }}</td>
              ...
              <td class="px-2 py-2 border text-right font-bold bg-yellow-50">
                <p>{{ item.stocks ?? 0 }} {{ item.unitAbbre }}</p>
              </td>
...
const listItems = async () => {
  try {
    ...
    setTimeout(async () => {
      ...
      await generateBarcodesForItems(response.data.items);
    }, 500);
  } catch (error) {
