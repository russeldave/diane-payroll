<template>
  <button
    ...
    type="button"
    :class="[
      'bg-yellow-500 text-white hover:opacity-70 rounded focus:bg-blue-300 font-bold text-sm md:text-md py-2 px-4 float-end text-nowrap ',
      className,
    ]"
  >
  ...
  <Modal
    ...
    :maxWidth="'xl'"
    title="View Returned Proof Images"
  ...
  >
    <div class="grid grid-cols-1 gap-6 p-6">
      ...
      <div v-else>
        <p class="text-center text-gray-500">No images available for this retured item.</p>
      </div>
    </div>
  </Modal>
</template>
...
const fetchImages = async () => {
  try {
    const response = await axios.post(
      `${VUE_APP_API_URL}return-items/get-images`,
      { return_item_id: props.data.id },
      BearToken(token)
    );
