      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm">
          ...
          >
            ...
            >
              <p>{{ Number(mrf.prQuantitys).toLocaleString() }} {{ mrf.unitAbbre }}</p>
            </td>
            ...
            <td class="px-2 py-3 border">
              <div class="flex">
                ...
                /> -->
                <Undo
                  v-if="hasPermission('MRF_Slip_Item_Undo_Rejected_Button')"
                  :key="mrf.id"
                  :mrf_item_id="mrf.id"
                  @transaction_id="handleTransaction()"
                />
...
const listMRFs = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}mrfs/list-mrf-items-rejecteds`,
      formData,
      BearToken(token)
    );
