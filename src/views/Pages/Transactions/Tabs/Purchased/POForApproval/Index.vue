          ...
          >
            ...
            <td class="px-2 py-1 border">
              <div class="flex justify-center">
                <View v-if="hasPermission('PO_Slip_View_Button')" :key="ri.id" :data="ri" @transaction_id="handleTransaction()" />
                <EditPO v-if="hasPermission('PO_Slip_Edit_Button')" :key="ri.id" :data="ri" @transaction_id="handleTransaction()" />
                <DraftPOSlip v-if="hasPermission('PO_Slip_View_Button')" :key="ri.id" :po_id="ri.id" @transaction_id="handleTransaction()" />
                <Approve v-if="hasPermission('PO_Approve_Button')" :key="ri.id" :po_id="ri.id" @transaction_id="handleTransaction()" />
              </div>
            </td>
          </tr>
...
import DraftPOSlip from "./Actions/DraftPOSlip.vue";
import EditPO from "../POForDelivery/Actions/EditPO.vue";
...
const listFDs = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}pos/list-for-approval`,
      formData,
      BearToken(token)
    );
