  ...
  >
    ...
    <div class="flex flex-col md:flex-row md:justify-between">
      ...
      <div class="flex justify-end">
        <ReceiveItem
          v-if="hasPermission('PO_Receive_Item_Button')"
          :data="fd_data"
          :po_item_array="po_item_array"
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200 text-xs md:text-sm">
          ...
          <tr
            ...
            :class="pr.selected ? 'bg-orange-100' : 'bg-white'"
          >
            ...
            <td class="px-2 py-3  border text-right bg-yellow-500  font-bold text-white">
              <u class="text-md md:text-lg">{{ pr.quantity ?? 0 }} {{ pr.unitAbbre }}</u>
            </td>
            <td class="px-2 py-3  border text-right bg-teal-500  font-bold text-white">
              <p >{{ pr.receivedQuantity ?? 0 }} {{ pr.unitAbbre }}</p>
            </td>
            <td class="px-2 py-3  border">{{ pr.remarks }}</td>
            <!-- <td class="px-2 py-3  border">{{ mrfStatusFormatter(mrf.status) }}</td> -->
            <td class="px-2 py-3  border">
              <div class="flex justify-center">
                <!-- <View v-if="hasPermission('PO_Slip_View_Item_Button')" :data="pr" @transaction_id="handleTransaction()" /> -->
                <Completed  v-if="hasPermission('PO_Slip_Completed_Item_Button')" :data="pr" @transaction_id="handleTransaction()"/>
              </div>
            </td>
          </tr>
...
//controls
import Completed from "../POForDelivery/Actions/Completed.vue";
...
const listFDs = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}pos/list-po-items-for-receiving`,
      formData,
      BearToken(token)
    );
...
const selectedItemID = (data, selected) => {
  ...
  if (selected) {
    ...
    if (!po_item_array.value.includes(data.id)) {
      po_item_array.value.push(data.id);
    }
  } else {
    // If selected is false, remove the item from fd_data.value
    const indexToRemove = fd_data.value.findIndex(
    ...
    const poItemIndexToRemove = po_item_array.value.indexOf(data.id);
    if (poItemIndexToRemove !== -1) {
      po_item_array.value.splice(poItemIndexToRemove, 1); // Remove itemId from item_array.value array
    }
  }
};
