<template>
  <button
    ...
    class="bg-yellow-500 hover:opacity-70 focus:bg-blue-300 text-white font-bold rounded text-sm md:text-md py-2 px-4 rounded-r mb-4 float-end text-nowrap"
  >
  ...
  <Modal
    ...
    :maxWidth="'2xl'"
    title="Edit Direct Receive Item"
    ...
    <div class="block gap-6 p-6">
      <form class="mt-0 grid grid-cols-2 gap-6" @submit.prevent="directReceiveItem()">
        ...
        <div class="mb-2 pt-4">
          ...
          >
          <!-- building_id,contact_info,address,geo_map,remarks. -->
          <div class="flex border border-2 rounded p-2 bg-gray-200">
          ...
          <div class="flex flex-col p-2 border border-2 border-orange-300 rounded-md">
            <label for="LeagueName" class="block font-bold text-md text-red-700">
              Override Quantity
            </label>
            <div class="flex flex-row">
              <input
                  type="number"
                  id="ProductName"
                  v-model="receiveForm.deducted_quantity"
                  placeholder="Deducted quantity"
                  class="mt-1 p-2 border rounded-md w-full"
                />
                <button
                  type="submit"
                  class="bg-red-500 text-white font-bold py-2 px-4 rounded-e hover:bg-red-600 hover:scale-110"
                  @click.prevent="overrideQuantity()"
                  >
...
const receiveItemBehavior = async () => {
  ...
  receiveForm.value = {
    ...
    recieving_price: parseFloat(data.quantity * data.price),
    warehouse_id: data.warehouseId,
    remarks: "", // if any
    expiration_date: data.expirationDate,
    unit_abbre: data.unitAbbre,
    direct_received_id: data.id
    
  }
  isReceiveItemModalOpen.value = true;
  console.log('receivedItemBehavior')
  console.log()
};
...
const directReceiveItem = async () => {
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}direct-receiveds/edit`,
      formData,
      BearToken(token)
    );
...
const overrideQuantity = async () => {
  try {
    ...
    const formData = FormDx(receiveForm.value);
    const response = await axios.post(
      `${VUE_APP_API_URL}direct-receiveds/edit-quantity`,
      formData,
      BearToken(token)
    );
