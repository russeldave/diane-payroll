<template>
  <button
    ...
    type="button"
    :class="[
      'bg-blue-500 rounded text-nowrap text-white hover:opacity-70 focus:bg-blue-300 font-bold text-sm md:text-md py-2 px-4 float-end text-nowrap ',
      className,
    ]"
  >
  ...
  <Modal
    ...
    :maxWidth="'4xl'"
    title="View Direct Receive Proof Images"
  ...
  >
    <div class="grid grid-cols-1 gap-6 p-6">
      ...
      <div v-else>
        <p class="text-center text-gray-500">No images available for this ingredient.</p>
      </div>
    </div>
  </Modal>
</template>
...
const fetchImages = async () => {
  try {
    const response = await axios.post(
      `${VUE_APP_API_URL}direct-receiveds/get-images`,
      { received_id: props.data.id },
      BearToken(token)
    );
