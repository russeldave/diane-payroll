...
const uploadUtakInventory = async () => {
  ...
  try {
    ...
    const response = await axios.post(
      `${VUE_APP_API_URL}ingredient-upload-excel/physical-count`,
      formData,
      BearToken(token),
      {
    ...
    if (error.response.data.error) {
      isUploadError.value = error.response.data.error;
      incompleteIngredients.value = isUploadError.value
        ? error.response.data.ingredientsNotFound
        : [];
    }
    isUploading.value = false;
    if (excelUpload.value) {
