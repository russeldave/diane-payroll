<template>
  <button
    @click.prevent="overrideButcherName()"
    title="Override Butcher Name"
...
const overrideButcherName = async () => {
  try {
    const confirmed = await Swal.fire({
      title: "Are you sure?",
      text: "You are about to override production name?. This action cannot be undone.",
      icon: "warning",
    ...
    if (confirmed.isConfirmed) {
      ...
      Swal.fire({
        title: "Processing...",
        text: "Please wait while we update the butcher name.",
      ...
      const formData = FormDx({
        item_id_number: props.data.ingredient_id_number,
        ingredient_name: props.data.ingredient_name,
      });

      const response = await axios.post(
        `${VUE_APP_API_URL}override-food/butchery-name`,
        formData,
        BearToken(token)
      );
