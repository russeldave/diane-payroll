<template>
  <button
    @click.prevent="overrideCostPrice()"
...
const props = defineProps({
  ingredient_id_butcher: [Number, String],
  cost_price: Number,
});
const overrideCostPrice = async () => {
  try {
    ...
    if (confirmed.isConfirmed) {
      ...
      const formData = FormDx({
        ingredient_id: props.ingredient_id_butcher,
        cost_price: props.cost_price,
      });

      const response = await axios.post(
        `${VUE_APP_API_URL}override-food/ingredient-butchery-production-cost-price`,
        formData,
        BearToken(token)
      );
